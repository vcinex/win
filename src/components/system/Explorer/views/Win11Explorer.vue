...
// ⚠️ 修复点：将多选状态和 clearSelection 提前声明 ⚠️
// === 多选状态 ===
...
// === 核心数据加载 (异步化改造) ===
const rawContents = ref<FileMetadata[]>([]);
const loadDirectory = async () => {
  try {
    rawContents.value = await readDirectory(currentPath.value);
  } catch (error) {
...
// 监听文件系统底层变化，自动热更新当前视图
let unwatchDir: (() => void) | null = null;
watch(
  currentPath,
  (newPath) => {
    if (unwatchDir) unwatchDir();
    unwatchDir = onFileChange(newPath, () => loadDirectory());
    loadDirectory();
    clearSelection(); // 此时 clearSelection 已经初始化，不再报错
  },
  { immediate: true }
);
...
const selectedItemMeta = computed(() =>
  firstSelectedItem.value
    ? rawContents.value.find((i) => i.path === firstSelectedItem.value) || null
    : null
);

// 获取当前目录过滤后的展示列表
const displayedContents = computed(() => {
  let list = [...rawContents.value];
...
// === 智能防重名 (异步化) ===
const getSafePath = async (dir: string, baseName: string, ext: string = '') => {
  let name = ext ? `${baseName}${ext}` : baseName;
  let counter = 1;
  let testPath = dir === '' ? name : `${dir}/${name}`;

  while (await exist(testPath)) {
    counter++;
    name = ext ? `${baseName} (${counter})${ext}` : `${baseName} (${counter})`;
    testPath = dir === '' ? name : `${dir}/${name}`;
  }
  return testPath;
};
...
const handleNewFolder = async () => {
  contextMenu.value.visible = false;
  newMenuVisible.value = false;
  const safePath = await getSafePath(currentPath.value, '新建文件夹');
  const defaultName = safePath.split('/').pop();
  const name = prompt('新建文件夹名称:', defaultName);
  if (name) {
    await createDirectory(currentPath.value === '' ? name : `${currentPath.value}/${name}`);
  }
};
...
const handleNewFile = async () => {
  contextMenu.value.visible = false;
  newMenuVisible.value = false;
  const safePath = await getSafePath(currentPath.value, '新建文本文档', '.txt');
  const defaultName = safePath.split('/').pop();
  const name = prompt('新建文档名称:', defaultName);
  if (name) {
    await writeFile(currentPath.value === '' ? name : `${currentPath.value}/${name}`, '');
  }
};
...
const handlePaste = async () => {
  ...
  for (const srcPath of paths) {
    ...
    if ((await exist(targetPath)) && type === 'copy') {
      const parts = fileName.split('.');
      const ext = parts.length > 1 ? `.${parts.pop()}` : '';
      const baseName = `${parts.join('.')} - 副本`;
      targetPath = await getSafePath(currentPath.value, baseName, ext);
    }
...
const handleMoveItems = async (sourcePaths: string[], targetDir: string) => {
  for (const src of sourcePaths) {
    ...
    await rename(src, `${targetDir}/${fileName}`);
  }
  clearSelection();
};
...
const handleDeleteSelected = async () => {
  ...
  if (selectedItems.value.size && confirm(`确定要永久删除这 ${selectedItems.value.size} 项吗？`)) {
    for (const p of Array.from(selectedItems.value)) {
      const meta = rawContents.value.find((i) => i.path === p);
      if (meta?.type === 'directory') await rmdir(p, true);
      else await unlink(p);
    }
    clearSelection();
  }
...
// 重命名逻辑
const handleRenameSelected = async (newName?: string | Event) => {
  contextMenu.value.visible = false;
  ...
  if (typeof newName === 'string' && newName.trim()) {
    ...
    if (finalName && finalName !== oldName) {
      ...
      await rename(targetPath, basePath + finalName);
      clearSelection();
      selectItem({ path: basePath + finalName, ctrlKey: false, shiftKey: false });
    }
  } else {
    if (mainContentRef.value) mainContentRef.value.startRename(targetPath);
  }
};
    ...
    switch (e.key) {
      ...
      case 'Enter':
        if (selectedItems.value.size === 1) {
          const meta = rawContents.value.find((i) => i.path === firstSelectedItem.value!);
